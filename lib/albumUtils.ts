 ...
 * @param addCredit Whether to add the creator credit to the bottom of the album.
 * @returns A promise that resolves to a data URL of the generated album page (JPEG format).
 */
export async function createAlbumPage(imageData: Record<string, string>, addCredit: boolean = false): Promise<string> {
    ...
    // 5. Add Creator Credit if requested
    if (addCredit) {
        ctx.save();
